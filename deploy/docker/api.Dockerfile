...
COPY src/LeadRecovery.Api/*.csproj src/LeadRecovery.Api/packages.lock.json src/LeadRecovery.Api/
...
RUN dotnet restore src/LeadRecovery.Api/LeadRecovery.Api.csproj --locked-mode

COPY src/ src/
RUN dotnet publish src/LeadRecovery.Api/LeadRecovery.Api.csproj \
    --configuration Release \
    --no-restore \
    --output /app/publish \
    /p:UseAppHost=false

FROM mcr.microsoft.com/dotnet/aspnet:10.0.10@sha256:f1126d438ccc359f51cc6d4701a8deae513856cf10f5fe645d29ea6403dcac6b AS runtime
ARG VERSION=0.0.0-local
ARG REVISION=unknown
ARG CREATED=unknown
LABEL org.opencontainers.image.title="LeadRecovery API" \
      org.opencontainers.image.description="LeadRecovery HTTP API and database migration runner" \
      org.opencontainers.image.source="https://github.com/Yashraj-Rathore/LeadRecovery" \
      org.opencontainers.image.version="$VERSION" \
      org.opencontainers.image.revision="$REVISION" \
      org.opencontainers.image.created="$CREATED"

RUN mkdir --parents /keys \
    && chown app:app /keys
...
ENTRYPOINT ["dotnet", "LeadRecovery.Api.dll"]
